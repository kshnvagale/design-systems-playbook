...
- **Focus indicators** - two separate criteria at two different levels, and conflating them gates AA work on a AAA rule. **SC 2.4.11 Focus Not Obscured (Minimum) is Level AA**: the focused component must not be entirely hidden by author-created content, and it is required at the WCAG 2.2 AA bar this skill defaults to. **SC 2.4.13 Focus Appearance is Level AAA**: minimum area, thickness, and contrast for the indicator itself against both the unfocused and focused states - a target worth designing toward, not an AA gate. Either way, a design system's `border.focus` token and focus-ring component behavior should be centrally guaranteed - don't leave this to individual product teams.
...
- **Dark mode contrast** needs its own pass, not an assumption that "light mode passed, so dark mode will too" - re-check every surface, border, and text pair against the dark base rather than inferring compliance from the light theme. `tokens.md` is the single source on the 15.8:1 base-surface figure, including what that number does and does not justify.

## Palette composition for a real product

A production palette needs, at minimum:

1. **Primary/brand** - the one hue used to signal "this is interactive/on-brand." Often the accessibility bottleneck: bright yellows, limes, and light oranges frequently fail 4.5:1 as text/icon color on white. **Standard fix**: don't force the literal brand hue to serve as your accessible "primary action" color. Either (a) use a darker step from the same hue family for text/icon/interactive-foreground use and reserve the literal brand hue for large fills/logos/marketing surfaces, or (b) treat brand color as decorative-only and pick a separate, accessible "action" color for interactive elements. Two things to be honest about with (a). Which way chroma moves depends on the hue: darker-and-more-saturated holds for blues and greens, but for yellows and limes the accessible step is *lower* in chroma, not higher. And the accessible step is often far enough down the lightness axis that it no longer reads as the brand color - `#FFD500` sits at 1.42:1 on white and first clears 4.5:1 at roughly `#8C7500`, a dark olive a stakeholder will not accept as their bright yellow. A shared ramp keeps the family relationship, it does not preserve brand identity at every step, so name that trade-off before the palette ships rather than in review. A useful discipline either way: reserve the brand hue for elements the user can actually act on, never for decorative fills, so "brand-colored" and "interactive" stay the same signal.
